...
public class ConnectionManager implements ConnectListener {
	...
	public static class StateIndication {
		...
		public StateIndication(ConnectionState state) {
			this(state, null);
		}
	...
	/*************************************
	 * a class encapsulating currentState machine
	 * information for a given currentState
	 *************************************/
	...
	public abstract class State {
		...
		abstract StateIndication onTimeout();
	...
	class Closing extends State {
		...
		void enact(StateIndication stateIndication, ConnectionStateChange change) {
			...
			if(closed) {
				addAction(new DeferredStateChangeAction(ConnectionState.closed));
			}
		}
	}
	...
	private static class DeferredActionQueue extends ArrayDeque<Action> {
	...
	private synchronized void requestState(ITransport transport, StateIndication stateIndication) {
		...
		addAction(new DeferredStateChangeAction(transport, stateIndication));
	}
	...
	private class SynchronousStateChangeAction extends StateChangeAction {
	...
	private class DeferredStateChangeAction extends StateChangeAction {
		DeferredStateChangeAction(ConnectionState state) {
			super(null, new StateIndication(state, null));
		}

		DeferredStateChangeAction(ITransport transport, StateIndication stateIndication) {
			super(transport, stateIndication);
		}
	...
	private abstract class StateChangeAction implements Action {
			...
			while(true) {
				...
				synchronized(ConnectionManager.this) {
					while(deferredActions.size() == 0) {
						...
						Action act = deferredActions.peek();
						if (act != null) {
				...
				while((deferredAction = deferredActions.poll()) != null) {
					try {
						deferredAction.run();
					} catch(Exception e) {
	...
	private final DeferredActionQueue deferredActions = new DeferredActionQueue();
